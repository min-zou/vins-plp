...
buffer大小为10+1，最新来的帧(x)放到最新的位置，检查它与上一帧(J)之间的视差情况
如果视差 > 阈值，则删除(A)，然后把(x)加入优化滑窗，并作为loopClosure中的KF。然后marginalization_flag = MARGIN_OLD
          否则，删除(J)，把(I)作为loopClosure中的KF。然后marginalization_flag = MARGIN_SECOND_NEW
*/
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    for (auto &id_pts : image)
    {
        ...
        if(id_pts.second.size() == 2)//双目的情况
        {
            f_per_fra.rightObservation(id_pts.second[1].second);// 双目的右目情况
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        if(STEREO && it_per_id.feature_per_frame[0].is_stereo)
        {
            ...
            Eigen::Matrix3d R1 = Rs[imu_i] * ric[1];//利用imu的位姿计算右相机位姿R
...
double FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id, int frame_count)
{
    ...
    p_i_comp = p_i;
    double dep_i = p_i(2);
    double u_i = p_i(0) / dep_i;// 转换到像素坐标下
