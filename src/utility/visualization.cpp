...
ros::Publisher pub_path, pub_odom_path;
ros::Publisher pub_point_cloud, pub_margin_cloud, pub_lines;
ros::Publisher pub_key_poses;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose_visual;
nav_msgs::Path path, odom_path;
                   ...
                   Eigen::Matrix3d loop_correct_r)
{
    ...
    for (auto &it_per_id : estimator.line_f_manager.line_feature)
    {
//        int used_num;
...
//        //if (it_per_id.start_frame > WINDOW_SIZE * 3.0 / 4.0 || it_per_id.solve_flag != 1)
        ...
        line_w.head(3) = nw;
        line_w.tail(3) = vw;
        auto Rwc = estimator.Rs[imu_i]*estimator.ric[0];
        auto twc = estimator.Ps[imu_i]+estimator.Rs[imu_i]*estimator.tic[0];
        ...
        auto obs = it_per_id.line_feature_per_frame.front().pts_s;   // 第一次观测到这帧
        Vector3d p11 = Vector3d(obs(0), obs(1), 1.0);
        obs = it_per_id.line_feature_per_frame.front().pts_e;   // 第一次观测到这帧
        Vector3d p21 = Vector3d(obs(0), obs(1), 1.0);
...
void pub_odomtopic_path(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, double t)
{
    geometry_msgs::PoseStamped pose_stamped;
